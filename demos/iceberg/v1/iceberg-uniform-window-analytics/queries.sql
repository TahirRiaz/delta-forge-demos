...
ASSERT ROW_COUNT = 40
SELECT * FROM {{zone_name}}.iceberg_window.sales ORDER BY sale_id;
...
FROM (
    ...
    FROM {{zone_name}}.iceberg_window.sales
) ranked
WHERE rn = 1
...
CREATE EXTERNAL TABLE IF NOT EXISTS {{zone_name}}.iceberg_window.sales_iceberg
USING ICEBERG
LOCATION '{{data_path}}/sales';

GRANT ADMIN ON TABLE {{zone_name}}.iceberg_window.sales_iceberg TO USER {{current_user}};
...
ASSERT ROW_COUNT = 40
SELECT * FROM {{zone_name}}.iceberg_window.sales_iceberg ORDER BY sale_id;
...
FROM (
    SELECT
        ...
        sale_amount,
        ROW_NUMBER() OVER (PARTITION BY rep_name ORDER BY sale_amount DESC) AS rn
    FROM {{zone_name}}.iceberg_window.sales_iceberg
) ranked
WHERE rn = 1
ORDER BY sale_amount DESC;
