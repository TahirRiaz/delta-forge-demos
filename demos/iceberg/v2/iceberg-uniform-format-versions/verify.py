...
def verify_sensors_v2(data_root, verbose=False):
    print_section("sensors_v2 — Iceberg V2 Format")
...
def verify_sensors_v3(data_root, verbose=False):
    print_section("sensors_v3 — Iceberg V3 Format")
    ...
    # Same as V1/V2 — this verifies the initial state before DML
    import pyarrow.compute as pc
    for loc, expected_avg in [("Lab-A", 23.08), ("Lab-B", 21.12), ("Lab-C", 22.43)]:
        mask = pc.equal(table.column("location"), loc)
        filtered = table.filter(mask)
...
def verify_cross_table(data_root, verbose=False):
    ...
    # All tables should have the same temperature sum
    sums = {v: round(pc.sum(t.column("temperature")).as_py(), 2) for v, t in tables.items()}
    if len(set(sums.values())) == 1:
        ok(f"All tables have SUM(temperature) = {sums['v1']}")
    else:
        fail(f"SUM(temperature) mismatch: {sums}")

    # All tables should have the same humidity sum
    ...
    # All tables should have the same column names
    cols = {v: t.column_names for v, t in tables.items()}
    if cols["v1"] == cols["v2"] == cols["v3"]:
        ok(f"All tables have identical column names: {cols['v1']}")
    else:
        fail(f"Column name mismatch: {cols}")
