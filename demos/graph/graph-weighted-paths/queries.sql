...
USE {{zone_name}}.graph.shipping_network
MATCH (p)
...
FROM {{zone_name}}.graph.routes r
JOIN {{zone_name}}.graph.ports src_p ON r.src = src_p.id
JOIN {{zone_name}}.graph.ports dst_p ON r.dst = dst_p.id
...
SELECT
    (SELECT COUNT(*) FROM {{zone_name}}.graph.ports)                                                 AS total_ports,
    (SELECT COUNT(*) FROM {{zone_name}}.graph.routes)                                                AS total_routes,
    (SELECT COUNT(*) FROM {{zone_name}}.graph.routes WHERE route_type = 'trunk')                     AS trunk_routes,
    (SELECT COUNT(*) FROM {{zone_name}}.graph.routes WHERE route_type = 'feeder')                    AS feeder_routes,
    (SELECT COUNT(*) FROM {{zone_name}}.graph.routes WHERE route_type = 'transshipment')             AS transshipment_routes;
