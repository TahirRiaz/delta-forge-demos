...
-- ============================================================================
CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.graph.physicians (
    id                  BIGINT,
    name                STRING,
    specialty           STRING,
    hospital            STRING,
...
GRANT ADMIN ON TABLE {{zone_name}}.graph.physicians TO USER {{current_user}};

INSERT INTO {{zone_name}}.graph.physicians
SELECT
    id,
    'Dr. ' || CASE (id % 10)
...
-- ============================================================================
CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.graph.referrals (
    id              BIGINT,
    src             BIGINT,
    dst             BIGINT,
    weight          DOUBLE,
...
GRANT ADMIN ON TABLE {{zone_name}}.graph.referrals TO USER {{current_user}};

-- Batch 1: Intra-hospital referrals (stride 3, same hospital) — 30 edges
INSERT INTO {{zone_name}}.graph.referrals
SELECT
...
WHERE src != dst
  AND (src % 6) != (dst % 6)
  AND NOT EXISTS (
      SELECT 1 FROM {{zone_name}}.graph.referrals r
...
-- GRAPH DEFINITION
-- ============================================================================
CREATE GRAPH IF NOT EXISTS {{zone_name}}.graph.hospital_referrals
    VERTEX TABLE {{zone_name}}.graph.physicians ID COLUMN id NODE TYPE COLUMN specialty NODE NAME COLUMN name
    EDGE TABLE {{zone_name}}.graph.referrals SOURCE COLUMN src TARGET COLUMN dst
