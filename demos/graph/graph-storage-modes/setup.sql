...
CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.graph.persons_flat (
    id          BIGINT,
    name        STRING,
    age         INT,
    department  STRING,
...
GRANT ADMIN ON TABLE {{zone_name}}.graph.persons_flat TO USER {{current_user}};

INSERT INTO {{zone_name}}.graph.persons_flat
SELECT
    id,
    CASE (id % 10)
...
CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.graph.edges_flat (
    id                  BIGINT,
    src                 BIGINT,
    dst                 BIGINT,
    weight              DOUBLE,
...
GRANT ADMIN ON TABLE {{zone_name}}.graph.edges_flat TO USER {{current_user}};

-- Batch 1: Intra-department colleagues (~80 edges, stride 5)
INSERT INTO {{zone_name}}.graph.edges_flat
SELECT
...
CREATE GRAPH IF NOT EXISTS {{zone_name}}.graph.storage_flat
    VERTEX TABLE {{zone_name}}.graph.persons_flat ID COLUMN id NODE TYPE COLUMN department NODE NAME COLUMN name
    EDGE TABLE {{zone_name}}.graph.edges_flat SOURCE COLUMN src TARGET COLUMN dst
...
SELECT
    ...
    '}' AS extras
FROM {{zone_name}}.graph.persons_flat;

CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.graph.edges_hybrid (
    src                 BIGINT,
    dst                 BIGINT,
    weight              DOUBLE,
    relationship_type   STRING,
...
GRANT ADMIN ON TABLE {{zone_name}}.graph.edges_hybrid TO USER {{current_user}};

INSERT INTO {{zone_name}}.graph.edges_hybrid
SELECT
    ...
    '}' AS extras
FROM {{zone_name}}.graph.edges_flat;

-- Graph definition: HYBRID
CREATE GRAPH IF NOT EXISTS {{zone_name}}.graph.storage_hybrid
    VERTEX TABLE {{zone_name}}.graph.persons_hybrid ID COLUMN id NODE TYPE COLUMN label NODE NAME COLUMN name
    EDGE TABLE {{zone_name}}.graph.edges_hybrid SOURCE COLUMN src TARGET COLUMN dst
    WEIGHT COLUMN weight
    EDGE TYPE COLUMN relationship_type
    DIRECTED
...
CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.graph.edges_json (
    src                BIGINT,
    dst                BIGINT,
    weight             DOUBLE,
...
GRANT ADMIN ON TABLE {{zone_name}}.graph.edges_json TO USER {{current_user}};

INSERT INTO {{zone_name}}.graph.edges_json
SELECT
    src, dst, weight, relationship_type,
    '{"since_year": ' || CAST(since_year AS VARCHAR) ||
    ', "frequency": "' || frequency ||
...
CREATE GRAPH IF NOT EXISTS {{zone_name}}.graph.storage_json
    VERTEX TABLE {{zone_name}}.graph.persons_json ID COLUMN id NODE TYPE COLUMN label
    EDGE TABLE {{zone_name}}.graph.edges_json SOURCE COLUMN src TARGET COLUMN dst
    WEIGHT COLUMN weight
    EDGE TYPE COLUMN relationship_type
    DIRECTED
