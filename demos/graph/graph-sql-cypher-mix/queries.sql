...
USE {{zone_name}}.sales.customer_network
MATCH (n)
...
FROM cypher('{{zone_name}}.sales.customer_network', $$
    CALL algo.pageRank({dampingFactor: 0.85, iterations: 20})
    YIELD node_id, score
    RETURN node_id AS customer_id, score
$$) AS (customer_id BIGINT, score DOUBLE) pr
JOIN {{zone_name}}.sales.customers c ON pr.customer_id = c.id
...
INSERT INTO {{zone_name}}.sales.influence_scores
SELECT * FROM cypher('{{zone_name}}.sales.customer_network', $$
    CALL algo.pageRank({dampingFactor: 0.85, iterations: 20})
...
FROM {{zone_name}}.sales.influence_scores i
JOIN {{zone_name}}.sales.customers c ON i.customer_id = c.id
...
INSERT INTO {{zone_name}}.sales.community_assignments
SELECT * FROM cypher('{{zone_name}}.sales.customer_network', $$
    CALL algo.louvain()
...
FROM {{zone_name}}.sales.influence_scores i
JOIN {{zone_name}}.sales.customers c ON i.customer_id = c.id
JOIN {{zone_name}}.sales.orders o ON c.id = o.customer_id
...
FROM (
    ...
    FROM {{zone_name}}.sales.community_assignments ca
    JOIN {{zone_name}}.sales.orders o ON ca.customer_id = o.customer_id
...
FROM {{zone_name}}.sales.sales_reps sr
JOIN {{zone_name}}.sales.customers c ON sr.territory = c.region
JOIN {{zone_name}}.sales.influence_scores i ON c.id = i.customer_id
JOIN {{zone_name}}.sales.community_assignments ca ON c.id = ca.customer_id
...
revenue AS (
    ...
    FROM {{zone_name}}.sales.orders
    GROUP BY customer_id
)
SELECT c.id, c.name, c.region, h.total_degree, r.total_revenue
FROM hub_scores h
JOIN {{zone_name}}.sales.customers c ON h.customer_id = c.id
...
ASSERT ROW_COUNT = 6
SELECT 'customers' AS entity, COUNT(*) AS row_count FROM {{zone_name}}.sales.customers
UNION ALL SELECT 'referrals', COUNT(*) FROM {{zone_name}}.sales.referrals
UNION ALL SELECT 'orders', COUNT(*) FROM {{zone_name}}.sales.orders
UNION ALL SELECT 'sales_reps', COUNT(*) FROM {{zone_name}}.sales.sales_reps
UNION ALL SELECT 'influence_scores', COUNT(*) FROM {{zone_name}}.sales.influence_scores
UNION ALL SELECT 'community_assignments', COUNT(*) FROM {{zone_name}}.sales.community_assignments
ORDER BY entity;
