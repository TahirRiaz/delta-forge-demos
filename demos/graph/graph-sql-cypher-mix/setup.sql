...
CREATE SCHEMA IF NOT EXISTS {{zone_name}}.sales
...
CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.sales.customers (
    id         BIGINT,
    name       STRING,
    region     STRING,
    industry   STRING,
...
GRANT ADMIN ON TABLE {{zone_name}}.sales.customers TO USER {{current_user}};

INSERT INTO {{zone_name}}.sales.customers VALUES
...
CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.sales.referrals (
    id             BIGINT,
    src            BIGINT,
    dst            BIGINT,
    weight         DOUBLE,
...
GRANT ADMIN ON TABLE {{zone_name}}.sales.referrals TO USER {{current_user}};

-- Batch 1: Regional partnerships (stride 4, same region)
INSERT INTO {{zone_name}}.sales.referrals VALUES
...
CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.sales.orders (
    order_id    BIGINT,
    customer_id BIGINT,
    amount      DOUBLE,
    product     STRING,
...
GRANT ADMIN ON TABLE {{zone_name}}.sales.orders TO USER {{current_user}};

-- Pass 1: orders 1-40 (one per customer)
INSERT INTO {{zone_name}}.sales.orders VALUES
...
CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.sales.sales_reps (
    rep_id    BIGINT,
    rep_name  STRING,
    territory STRING,
    quota     INT
...
GRANT ADMIN ON TABLE {{zone_name}}.sales.sales_reps TO USER {{current_user}};

INSERT INTO {{zone_name}}.sales.sales_reps VALUES
...
CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.sales.influence_scores (
    customer_id     BIGINT,
    influence_score DOUBLE,
    influence_rank  BIGINT
) LOCATION '{{data_path}}/sales/influence_scores';

GRANT ADMIN ON TABLE {{zone_name}}.sales.influence_scores TO USER {{current_user}};

CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.sales.community_assignments (
    customer_id  BIGINT,
    community_id BIGINT
) LOCATION '{{data_path}}/sales/community_assignments';

GRANT ADMIN ON TABLE {{zone_name}}.sales.community_assignments TO USER {{current_user}};
...
CREATE GRAPH IF NOT EXISTS {{zone_name}}.sales.customer_network
    VERTEX TABLE {{zone_name}}.sales.customers ID COLUMN id NODE TYPE COLUMN region NODE NAME COLUMN name
    EDGE TABLE {{zone_name}}.sales.referrals SOURCE COLUMN src TARGET COLUMN dst
