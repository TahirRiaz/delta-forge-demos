...
-- ============================================================================
CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.graph.researchers (
    id          BIGINT,
    name        STRING,
    department  STRING,
    rank        STRING,
...
GRANT ADMIN ON TABLE {{zone_name}}.graph.researchers TO USER {{current_user}};

INSERT INTO {{zone_name}}.graph.researchers
SELECT
    id,
    'Prof. ' || CASE (id % 10)
...
-- ============================================================================
CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.graph.collaborations (
    id              BIGINT,
    src             BIGINT,
    dst             BIGINT,
    weight          DOUBLE,
...
GRANT ADMIN ON TABLE {{zone_name}}.graph.collaborations TO USER {{current_user}};
...
INSERT INTO {{zone_name}}.graph.collaborations
SELECT
...
-- GRAPH DEFINITION
-- ============================================================================
CREATE GRAPH IF NOT EXISTS {{zone_name}}.graph.research_network
    VERTEX TABLE {{zone_name}}.graph.researchers ID COLUMN id NODE TYPE COLUMN department NODE NAME COLUMN name
    EDGE TABLE {{zone_name}}.graph.collaborations SOURCE COLUMN src TARGET COLUMN dst
