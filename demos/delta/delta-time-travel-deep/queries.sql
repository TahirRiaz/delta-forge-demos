...
-- ============================================================================
-- The sensor_readings table went through 4 versions:
--   V0: 40 initial readings (10 per location: lab-a, lab-b, warehouse, field)
--   V1: Calibrated lab-a sensors (reading += 2.5)
--   V2: Deleted 5 faulty field sensors (impossible readings like 99.9, -40.0)
--   V3: Added 15 new sensor readings
--
...
-- ============================================================================
-- VERSION AS OF 0 shows the table exactly as it was after the initial INSERT.
-- At V0, lab-a sensors had their original uncalibrated readings, and the
...
SELECT location,
       COUNT(*) AS sensors,
       ROUND(AVG(reading), 1) AS avg_reading
FROM {{zone_name}}.delta_demos.sensor_readings VERSION AS OF 0
GROUP BY location
ORDER BY location;
...
-- versions. Here we look at sensor S001 before calibration (V0: 22.5)
-- and after (V1+: 25.0). The delta of +2.5 confirms the calibration offset.

-- Verify S001 reading before calibration was 22.5 and after is 25.0
ASSERT VALUE reading = 22.5 WHERE version_label = 'Before calibration (V0)'
ASSERT VALUE reading = 25.0 WHERE version_label = 'After calibration (V1+)'
ASSERT ROW_COUNT = 2
SELECT 'Before calibration (V0)' AS version_label,
       sensor_id, reading
FROM {{zone_name}}.delta_demos.sensor_readings VERSION AS OF 0
WHERE sensor_id = 'S001'
UNION ALL
SELECT 'After calibration (V1+)',
       sensor_id, reading
FROM {{zone_name}}.delta_demos.sensor_readings
WHERE sensor_id = 'S001';
...
-- ============================================================================
-- At V1, the faulty field sensors still existed (they were deleted in V2).
...
--   V0: 40 (initial load)
--   V1: 40 (calibration changed values, not row count)
--   V2: 35 (5 faulty sensors deleted)
--   V3: 50 (15 new sensors added)
...
SELECT 'V0' AS version,
       (SELECT COUNT(*) FROM {{zone_name}}.delta_demos.sensor_readings VERSION AS OF 0) AS row_count
UNION ALL
SELECT 'V1',
       (SELECT COUNT(*) FROM {{zone_name}}.delta_demos.sensor_readings VERSION AS OF 1)
UNION ALL
SELECT 'V2',
...
-- ============================================================================
-- V3 added 15 new sensors (ids 41-55) across all 4 locations.
-- These only appear in V3, not in earlier versions.
...
-- ============================================================================
-- At V1, the field location had 10 sensors (5 good + 5 faulty).
-- After V2's DELETE, only 5 remain. V3 added 5 more replacements.

-- Verify field sensor counts: 10 at V1, 10 after delete + new additions
ASSERT VALUE field_sensors = 10 WHERE state = 'V1 (before delete)'
ASSERT VALUE field_sensors = 10 WHERE state = 'Current (after delete + new)'
ASSERT ROW_COUNT = 2
SELECT 'V1 (before delete)' AS state,
...
-- Verify version 0 had 40 rows
ASSERT VALUE v0_row_count = 40
SELECT COUNT(*) AS v0_row_count FROM {{zone_name}}.delta_demos.sensor_readings VERSION AS OF 0;
...
-- Verify version 1 still has 10 field sensors (before delete)
ASSERT VALUE v1_field_count = 10
SELECT COUNT(*) AS v1_field_count FROM {{zone_name}}.delta_demos.sensor_readings VERSION AS OF 1 WHERE location = 'field';
...
-- Verify version 3 is queryable
ASSERT VALUE v3_row_count = 50
SELECT COUNT(*) AS v3_row_count FROM {{zone_name}}.delta_demos.sensor_readings VERSION AS OF 3;
