...
--
-- Expected: 15 rows — 4 from order, 3 from planning, 4+4 from utility bills
...
--
-- Expected: 10 distinct types — ORDHDR:9, ORDERS:9, ORDTLR:9, PPRHDR:2,
...
--
-- Expected: 4 files — order(4), planning(3), utility_bill(4), escape(4)
...
--
-- Expected: 15 rows, but TYP_1 will be NULL for header/trailer messages
-- that lack a TYP segment
...
--
-- Expected: 4 rows (one per distinct STX envelope), dates span 1994-2018
...
--
-- Note: Row counts use >= to accommodate the TRADACOMS multi-message
-- format where each file produces multiple rows (one per MHD segment).

SELECT check_name, result FROM (

    -- Check 1: Total message count >= 4 (at least one row per file;
    -- if multi-MHD parsing is active, expect 15)
    SELECT 'message_count' AS check_name,
           CASE WHEN (SELECT COUNT(*) FROM {{zone_name}}.edi.tradacoms_messages) >= 4
    ...
    -- Check 4: Materialized table also has rows (same files, additional columns)
    SELECT 'materialized_count' AS check_name,
           CASE WHEN (SELECT COUNT(*) FROM {{zone_name}}.edi.tradacoms_materialized) >= 4
                THEN 'PASS' ELSE 'FAIL' END AS result

    UNION ALL

    -- Check 5: df_transaction_json is populated for at least 4 messages
