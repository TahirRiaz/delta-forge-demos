...
SELECT
    (SELECT COUNT(*) FROM {{zone_name}}.github_releases.rust_releases_bronze)
        - (SELECT COUNT(*) FROM {{zone_name}}.github_releases.rust_releases_silver)
                                                                                 AS row_count_delta,
    (SELECT COUNT(DISTINCT tag_name) FROM {{zone_name}}.github_releases.rust_releases_bronze)
...
SELECT
    ...
    CASE WHEN COUNT(*) = (SELECT COUNT(*) FROM {{zone_name}}.github_releases.rust_releases_bronze)
